...
from world import motion_diff, motion_omni, lidar2d
from collections import namedtuple
from util import collision_cir_cir, collision_cir_matrix, collision_cir_seg

class mobile_robot():

    def __init__(self, id, mode = 'diff', init_state=np.zeros((3, 1)), vel_diff = np.zeros((2, 1)), vel_omni = np.zeros((2, 1)), vel_max = 2* np.ones((2, 1)), goal = np.zeros((2, 1)), goal_threshold = 0.1, radius = 0.2, step_time = 0.1, reso=0.01, **kwargs):
        ...
        self.color = kwargs.get('color_list', [[255, 165, 0], [255, 215, 0], [128, 0, 128]])[self.id]
        self.arrive_flag = False
        self.collision_flag = False
        self.source_flag = False
        self.source_flag1 = False
    ...
    def collision_check(self, components):
        ...
        #         temp_circle = circle(source.source_state[0][0], source.source_state[1][0], source.radius)
        #         if collision_cir_cir(self_circle, temp_circle):
        #             self.collision_flag = True
        #             print(f'collisions between robots_{self.id} and source')
        #             return True
