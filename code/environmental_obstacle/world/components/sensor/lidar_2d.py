...
from util import range_seg_matrix, range_cir_seg, range_seg_seg
import random

class lidar2d:

    def __init__(self, reso=1, range_min=0, range_max=10, angle_min=0, angle_max=pi, number=36, scan_time=0.1, noise=True, std=0.2, install_pos=np.zeros(3,), **kwargs):
        self.reso = reso
        self.range_min = range_min * self.reso
        self.range_max = range_max * self.reso
    ...
    def seg_components(self, segment, components):
        ...
        #     flag, int_point, lrange = range_cir_seg(source.source_state[0:2, 0], source.radius/2, segment)
    ...
    def nearest_obstacle(self, state):
        min_dis = np.min(self.range_data)
        min_index = np.argmin(self.range_data)
        real_angle_list = state[2, 0] - np.pi / 2 + self.angle_list
        return min_dis, real_angle_list[min_index]
