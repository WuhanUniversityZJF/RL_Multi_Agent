...
def range_seg_matrix(segment, matrix, reso, point_step_weight=2, offset=np.zeros(2,)):
    ...
    while cur_len <= len_seg:
        ...
        elif not matrix[int(index[0]), int(index[1])]:
            lrange = np.linalg.norm( cur_point -  init_point)
...
def num_seg_matrix(segment, matrix, reso):
    ...
    init_point = segment[0]
    end_point = segment[1]
    # 使用Bresenham算法获取经过栅格的浮点数坐标
    indices = list(bresenham(int(init_point[0]/reso), int(init_point[1]/reso),
                                   int(end_point[0]/reso), int(end_point[1]/reso)))
    # # 转换为分辨率坐标
    # indices = [(x[0] * 5, x[1] * 5) for x in indices]
    num = 0
