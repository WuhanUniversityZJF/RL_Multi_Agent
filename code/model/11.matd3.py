    ...
    def compute_value_loss(self, args, agents, s_batch, a_batch, r_batch, d_batch, ns_batch):
        ...
        with torch.no_grad():
            ...
            for agent_id, agent in agents.items():
                a = agent.target_Mu(ns_batch[agent_id])
                ...
                # a = torch.clamp(a + noise, min=0, max=self.max_action)
                batch_a_next[agent_id] = a
            # 让两个目标价值网络做预测。
            ...
            # 计算 TD 目标。
            y = r_batch[self.id] + args.gamma * torch.min(q1, q2) * (1 - d_batch[self.id])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--algorithm", type=str, default="MATD3", help="MATD3 or MATD3_LSTM")
    ...
    parser.add_argument("--add_model", type=bool, default=True, help="Loading model")
    # --------------------------------------MATD3--------------------------------------------------------------------
    parser.add_argument("--policy_noise", type=float, default=0.1, help="Target policy smoothing")
    parser.add_argument("--noise_clip", type=float, default=0.25, help="Clip noise")
    ...
    agents = {agent_id:MATD3(max_action=args.max_action, num_states=observation_size, num_actions=num_actions, args=args, agent_id=agent_id) for agent_id in env.possible_agents}
